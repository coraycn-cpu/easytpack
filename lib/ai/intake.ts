...
- visibleGarments：列出画面中可识别的各单件（上装、下装、外套等分别列出，id 用 g1、g2…），最多 6 项单件；上装与下装必须拆开，不要把马甲短裤合成一项单件
- 每项 label/category 要具体（如「V领无袖马甲」「休闲短裤」），便于用户点选后只生成那一件
...
${locked ? "目标单款已锁定，不要再问品类，改为确认面料、版型、季节、工艺细节等。" : "根据已有信息，生成简单问题，用大白话提问。"}
规则：
